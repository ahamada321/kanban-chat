...
export class RentalRevenueComponent implements OnInit {
    ...
    if (payments) {
      for (let payment of payments) {
        if (moment(payment.paidAt).format('YYYY') == moment().format('YYYY')) {
          if (
            moment(payment.paidAt).format('MM') ==
            moment().month('December').format('MM')
          ) {
            revenue_Dec += payment.ownerRevenue;
          } else if (
            moment(payment.paidAt).format('MM') ==
            moment().month('November').format('MM')
          ) {
            revenue_Nov += payment.ownerRevenue;
          } else if (
            moment(payment.paidAt).format('MM') ==
            moment().month('October').format('MM')
          ) {
            revenue_Oct += payment.ownerRevenue;
          } else if (
            moment(payment.paidAt).format('MM') ==
            moment().month('September').format('MM')
          ) {
            revenue_Sep += payment.ownerRevenue;
          } else if (
            moment(payment.paidAt).format('MM') ==
            moment().month('August').format('MM')
          ) {
            ...
            moment(payment.paidAt).format('MM') ==
            moment().month('June').format('MM')
          ) {
            revenue_Jun += payment.ownerRevenue;
          } else if (
            moment(payment.paidAt).format('MM') ==
            moment().month('May').format('MM')
          ) {
            revenue_May += payment.ownerRevenue;
          } else if (
            moment(payment.paidAt).format('MM') ==
            moment().month('April').format('MM')
          ) {
            revenue_Apr += payment.ownerRevenue;
          } else if (
            moment(payment.paidAt).format('MM') ==
            moment().month('March').format('MM')
          ) {
            revenue_Mar += payment.ownerRevenue;
          } else if (
            moment(payment.paidAt).format('MM') ==
            moment().month('Febraury').format('MM')
          ) {
            revenue_Feb += payment.ownerRevenue;
          } else if (
            moment(payment.paidAt).format('MM') ==
            moment().month('January').format('MM')
          ) {
  ...
  smoothScroll(target) {
    ...
    this.scrollTo(
      ...
      targetScroll.offsetTop - this.headerOffset,
      625
    ); // Updated by Creative Tim support!
  }
