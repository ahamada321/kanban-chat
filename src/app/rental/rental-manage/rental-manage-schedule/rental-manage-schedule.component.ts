...
@Component({
  ...
  template: `
    ...
    <div class="modal-body">
      <div class="row">
        <div class="col-md-6 text-center">受付開始</div>
      ...
      <div class="row">
        <div class="col-md-6 text-center">受付終了</div>
      ...
      <div class="text-center">{{ date }}の受付時間を設定します</div>
    </div>
    <div class="modal-footer">
      <div class="left-side">
        <button
          ...
          (click)="activeModal.close(onSetClicked(startTime, endTime))"
        >
          セットする
        </button>
      </div>
...
export class RentalManageScheduleComponent implements OnInit, OnDestroy {
  ...
  handleDragSelect(arg) {
    // handler method
    if (
      arg.startStr == moment(arg.endStr).subtract(1, 'day').format('YYYY-MM-DD')
    ) {
      return;
    }
    Swal.fire({
